...
let bed_block_format = function(begin, duration, end){
  let beginIcon = iconHTML('step-backward');
...
let begin_format = function(begin){
  let beginIcon = iconHTML('step-backward');
  return `${begin?beginIcon+moment(begin).format("D.M.Y, h:mm a"):''}`
};
let end_format = function(end){
  let endIcon = iconHTML('step-forward')
  return `${end?endIcon+moment(end).format("D.M.Y, h:mm a"):''}`
};
let duration_format = function( duration){
  let durationIcon = iconHTML('hourglass');
  return `${duration?durationIcon+durationFormat(moment.duration(duration*1000).toISOString()):''}`
};
export { durationFormat, bed_block_format,begin_format,end_format,duration_format};
